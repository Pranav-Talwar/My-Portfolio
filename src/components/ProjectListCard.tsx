  ...
  return (
    <a href={project.href} className="project-card group block relative">
      <div className="p-8 md:p-12 flex flex-col md:flex-row gap-10 items-center transition-colors group-hover:bg-[var(--bg-card)]/60">
        {/* Image */}
        <div className="w-full md:w-2/5 rounded-lg overflow-hidden relative">
        ...
        <div className="w-full md:w-3/5 space-y-5">
          ...
          <div>
            <h3 className="text-2xl font-bold text-[var(--text-heading)] mb-2 group-hover:translate-x-1 transition-transform inline-flex items-center gap-3">
              {project.title}
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-1 bg-[var(--bg-surface)] border border-[var(--border)] rounded text-[10px] font-mono text-[var(--text-secondary)] hover:border-[var(--border-alt)] hover:text-[var(--text-heading)] transition-colors"
              >
                {tag}
              </span>
            ))}
